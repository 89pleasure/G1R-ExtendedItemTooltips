...
- `package/EquippedItemTooltips/Scripts/main.lua` contains all runtime mod logic.
- `package/EquippedItemTooltips/EquippedItemTooltips.ini` is the user-facing configuration file.
- `package/EquippedItemTooltips/enabled.txt` and `readme.txt` are packaged with the mod.
- `README.md` is the public project documentation.
- `assets/nexus/` stores Nexus Mods image assets.
- `package/EquippedItemTooltips.zip` is package output; avoid editing it by hand.
...
```powershell
Copy-Item .\package\EquippedItemTooltips `
...
```powershell
rg -n "Tooltip|Hover|ForceTooltipVisibility" package/EquippedItemTooltips
git diff
```
...
If a Lua interpreter is available, run a syntax check with `luac -p package/EquippedItemTooltips/Scripts/main.lua`.
